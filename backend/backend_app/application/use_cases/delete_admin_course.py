...
class DeleteAdminCourseUseCase:
    ...
    async def execute(self, *, token: str, course_id: int) -> bool:
        status_code, payload = await self.postgres_service.delete_admin_course(
            token=token,
            course_id=course_id,
        )
        if status_code == 401:
        ...
        if status_code == 403:
            raise ForbiddenError("Only admin can delete courses")
        if status_code == 404:
