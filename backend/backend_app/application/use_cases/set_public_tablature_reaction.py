...
class SetPublicTablatureReactionUseCase:
    ...
    async def execute(self, *, token: str, tablature_id: int, reaction_type: str) -> dict:
        status_code, payload = await self.postgres_service.set_public_tablature_reaction(
            token=token,
        ...
        if status_code == 400:
            raise ValidationError("Invalid reaction payload")
        if status_code != 200:
